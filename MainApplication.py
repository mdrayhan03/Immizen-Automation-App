...
class Application:
    ...
    def header(self):
        ...
        image_path = path.join(self.current_dir, 'assets', 'logo.png')    
    ...
    def folder_existance(self) :
        ...
        initial = path.join(self.current_dir, 'DocFile', 'Initial Consultation Agreement')
        if not path.exists(initial) :
            makedirs(initial)
        
        invoice = path.join(self.current_dir, 'DocFile', 'Invoice')
        if not path.exists(invoice) :
            makedirs(invoice)

        money = path.join(self.current_dir, 'DocFile', 'Money Receipt')
        if not path.exists(money) :
            makedirs(money)

        service = path.join(self.current_dir, 'DocFile', 'Service Agreement')
        if not path.exists(service) :
            makedirs(service)
        
        sheet = path.join(self.current_dir, 'DocFile', 'Sheet')
